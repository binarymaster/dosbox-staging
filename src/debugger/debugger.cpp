	...
	for (auto i = BPoints.begin(); i != BPoints.end(); ++i) {
		...
		if ((bp->GetType() == BKPNT_PHYSICAL) && bp->IsActive() &&
			...
			if (bp->GetOnce()) {
				...
				// Also look for once-only breakpoints at this
				// address
...
static void DrawCode(void)
{
	...
	for (int i = 0; i < layout_sizes[code] - 1; i++) {
		...
		if (drawsize > (uint32_t)layout_sizes[code] - 1) {
			toolarge = true;
			drawsize = layout_sizes[code] - 2;
		}
...
uint32_t DEBUG_CheckKeys(void)
{
	...
	if (key > 0 || numberrun) {
		...
		case KEY_DOWN: // down
			if (codeViewData.cursorPos < layout_sizes[code] - 2) {
...
static void LogMCBChain(uint16_t mcb_segment)
{
	...
	auto DOS_dataOfs = static_cast<uint16_t>(dataOfs);    // Realmode
	                                                      // addressing only
	PhysPt dataAddr = PhysicalMake(dataSeg, DOS_dataOfs); // location being
	                                                      // viewed in the
	                                                      // "Data Overview"
